...
cutoffs=(1000 5000 10000 13000 17000)  # Add your cutoff values here
looseness_values=(0)  # Looseness values from 0.1 to 1 in steps of 0.1
...
for database in "${databases[@]}"; do
    for cutoff in "${cutoffs[@]}"; do
            ...
            echo "$command" >> "experiments/log-bruteforce-${database}-${cutoff}-${current_time}.txt"

            # Display the command
            echo "Executing: $command"

            # Run the command for each combination of database, cutoff, and looseness
            { time eval "$command"; } 2>> "experiments/log-bruteforce-${database}-${cutoff}-${current_time}.txt"
        
        
        logfile="experiments/log-LP-${database}-${cutoff}-${current_time}.txt"
        echo "" > $logfile
        ...
        for looseness in "${looseness_values[@]}"; do
            ...
            echo "Looseness: $looseness" >> $logfile
            # Display the command
            echo "Executing: $command"

            # Run the command for each combination of database, cutoff, and looseness
            { time eval $command ; } 2>&1 | tee -a $logfile
            # /usr/bin/time -ao $logfile $command >> $logfile 2>&1
